...
function normalizeTime(value) {
  ...
  if (hour < 9 || hour > 19 || minute > 59 || minute % 5 !== 0 || (hour === 19 && minute !== 0)) {
...
function generateSlots(openHour = 9, lastBookingHour = 19, intervalMinutes = 5) {
  const slots = [];
  for (let minutes = openHour * 60; minutes <= lastBookingHour * 60; minutes += intervalMinutes) {
...
class BookingStore {
  ...
  getBookingForCancellation(bookingId, cancelToken) {
    const booking = this.db.prepare(`
      SELECT booking_id, customer_name, service, booking_date, booking_time, status
      FROM bookings
